...
#        errchou "Adding positional data [$a]";
  ...
  if [[ -v provided["--help"] ]]; then declare -r sanitize="echon"
  ...
  for name in "${!values[@]}"; do
    ...
    if [[ ! -v provided["$name"] ]]; then
      if   [[ -v required["$name"] ]]; then "$sanitize" "${text_blr}✖ missing $name$text_normal";
      elif [[ -v optional["$name"] ]]; then
        :;
      elif [[ -v defaulted["$name"] ]]; then
        :;
      ...
      :;
      ((_setargs_debug)) &&
        if [[ -v by_name["$name"] ]]; then errchod "Setting name $name by name";
        else errchod "Setting name $name by position";
        fi
    fi
