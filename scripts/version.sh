...
version_read()
{
  ...
  declare -i ret="";
  versioning_groups_string="$(trap - ERR; set +e;
...
version_satisfied()
{
  ...
  declare -ir patch_gt=$(( ${_expected_[2]} >  ${_provided_[2]} ));
                                          ...
                                          # Exp    Prov
  if   (( ! major_eq)); then res="false"; # 1.x.x  2.x.x  ✖
  elif (( ! minor_le)); then res="false"; # 1.2.x  1.1.x  ✖
  elif ((   minor_lt)); then res="true";  # 1.2.x  1.3.x  ✔
  #else                                   # 1.2.x  1.2.y
  elif ((   patch_lt)); then res="true";  # 1.2.3  1.2.4(-abc)  ✔ (Exp has no pre-release)
  # If there exists a smaller paatch version, that means they are both greater
  # or equal to patch version 0.
  elif ((   patch_gt)); then res="true";  # 1.2.3  1.2.2
...
version_print()
{
  ...
  if ((${#_pr1ntgrp_[3]} != 0));
  then declare -r pre="$a-$b";
  else declare -r pre="";
  fi
  if (( ${#_pr1ntgrp_[4]} != 0 ));
  then declare -r build="$a+$b";
  else declare -r build="";
  fi
...
function augment_version()
{
  ...
  if ((_debug_version)); then
    ...
    errchod "Call stack:";
    declare -i i=0; while caller $i; do ((++i)); done
