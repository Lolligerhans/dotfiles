...
if [[ ! "${dotfiles}" -ef "${HOME}/dotfiles" ]]; then
  1>&2 printf "%s\n" "[!] Loading external dotfiles $dotfiles"; fi
if [[ -v DOTFILES ]]; then
  1>&2 printf "%s\n" "[!] Receiving from temporary dotfiles=$dotfiles"; fi
...
command_interactive()
{
  ...
  select chosen_command in "drop" "${command_list[@]}"; do
    case "$chosen_command" in
      ...
      drop)
        if (($# == 0));
        then errchow "Already without arguments";
        else subcommand interactive; # Without arguments
        fi
        ;;
      interactive)
